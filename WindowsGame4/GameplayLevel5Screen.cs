...
{
    class GameplayLevel5Screen : GameScreen
    {
        ...
        public override void LoadContent()
        {
            ...
            enemiesUFO = new List<BackwardEnemy>();
            ...
            mainbackground.Initialize(content, "carrotField", ScreenManager.GraphicsDevice.Viewport.Width, -1);
            ...
            enemyUFO = content.Load<Texture2D>("UFO");
        ...
        public override void Draw(GameTime gameTime)
        {
            ...
            for (int i = 0; i < enemiesUFO.Count; i++)
            {
                enemiesUFO[i].Draw(spriteBatch);
            }
            ...
            spriteBatch.Begin();
            spriteBatch.DrawString(smallFont, "Level 5", new Vector2(ScreenManager.GraphicsDevice.Viewport.TitleSafeArea.X + 380, ScreenManager.GraphicsDevice.Viewport.TitleSafeArea.Y), Color.White);
            spriteBatch.DrawString(smallFont, "Goal : 20000", new Vector2(ScreenManager.GraphicsDevice.Viewport.TitleSafeArea.X + 720, ScreenManager.GraphicsDevice.Viewport.TitleSafeArea.Y), Color.White);
        ...
        private void UpdateEnemies(GameTime gameTime)
        {
            ...
            for (int i = enemiesUFO.Count - 1; i >= 0; i--)
            {
                enemiesUFO[i].Update();

                if (enemiesUFO[i].Active == false)
                {
                    enemiesUFO.RemoveAt(i);
                }
            }
        }
        ...
        private void UpdateCollison()
        {
            ...
            for (int x = 0; x < enemiesUFO.Count; x++)
            {
                enemyUFOSpace = new Rectangle((int)enemiesUFO[x].Position.X, (int)enemiesUFO[x].Position.Y, enemiesUFO[x].Width, enemiesUFO[x].Height);
                if (player.Intersects(enemyUFOSpace))
                {
                    score += enemiesUFO[x].Value;
                    animatedSprite.Health -= enemiesUFO[x].Damage;
                    enemiesUFO.RemoveAt(x);
                    if (animatedSprite.Health == 0)
                    {
                        ScreenManager.AddScreen(new GameOverScreen(), ControllingPlayer);
                    }
            ...
            for (int x = 0; x < projectiles.Count; x++)
            {
                ...
                for (int y = 0; y < enemiesRockStone.Count; y++)
                {
                    ...
                    if (projectile.Intersects(enemyRockStoneSpace))
                    {
                        ...
                        if (score >= 20000)
                        {
                            ScreenManager.AddScreen(new GameFinish(), ControllingPlayer);
                        }
                    }
                }
                ...
                for (int y = 0; y < enemiesUFO.Count; y++)
                {
                    enemyUFOSpace = new Rectangle((int)enemiesUFO[y].Position.X, (int)enemiesUFO[y].Position.Y, enemiesUFO[y].Width, enemiesUFO[y].Height);
                    if (projectile.Intersects(enemyUFOSpace))
                    {
                        score += enemiesUFO[y].Value;
                        enemiesUFO.RemoveAt(y);
                        projectiles.RemoveAt(x);
                        if (score >= 20000)
                        {
                            ScreenManager.AddScreen(new GameFinish(), ControllingPlayer);
